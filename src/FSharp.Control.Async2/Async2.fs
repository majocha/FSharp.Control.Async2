...
module internal SR =
    let mismatchIARCancel = "The IAsyncResult object provided does not match this 'Cancel' operation."
    let mismatchIAREnd = "The IAsyncResult object provided does not match this 'End' operation."
    let controlContinuationInvokedMultipleTimes =
        "A continuation provided by Async.FromContinuations was invoked multiple times"
    let maxDegreeOfParallelismNotPositive = "maxDegreeOfParallelism must be positive, was {0}"
    let failedReadEnoughBytes = "Failed to read enough bytes from the stream."

    let GetString value = value
...
type Async2Return =
    ...
    static member inline Fake() =
        Unchecked.defaultof<Async2Return>

type cont<'T> = 'T -> Async2Return
type econt = ExceptionDispatchInfo -> Async2Return
type ccont = OperationCanceledException -> Async2Return
...
type Trampoline() =
    ...
    [<DebuggerHidden>]
    member _.Execute(firstAction: unit -> Async2Return) =
        ...
        try
            ...
            while keepGoing do
                ...
                //
                // So when running async2 code, there aren't any intermediate catch handlers (though there
                ...
                with exn ->
                    ...
                    | None ->
                        ...
                        // direct uses of combinators (not using async2 {...}) may cause
                        // code to execute unprotected, e.g. async2.While((fun () -> failwith ".."), ...) executes the first
...
type TrampolineHolder() =
        ...
        | null ->
            sendOrPostCallbackWithTrampoline <-
                SendOrPostCallback(fun o ->
                    let f = unbox<unit -> Async2Return> o
                    // Reminder: the ignore below ignores an Async2Return.
    ...
    /// Execute an async2 computation after installing a trampoline on its synchronous stack.
    [<DebuggerHidden>]
    member _.ExecuteWithTrampoline firstAction =
        trampoline <- Trampoline()
        trampoline.Execute firstAction

    member this.PostWithTrampoline (syncCtxt: SynchronizationContext) (f: unit -> Async2Return) =
        syncCtxt.Post(getSendOrPostCallbackWithTrampoline this, state = (f |> box))
        Async2Return.Fake()

    member this.QueueWorkItemWithTrampoline(f: unit -> Async2Return) =
    ...
    member this.StartThreadWithTrampoline(f: unit -> Async2Return) =
    ...
    /// Call a continuation, but first check if an async2 computation should trampoline on its synchronous stack.
    member inline _.HijackCheckThenCall (cont: 'T -> Async2Return) res =
...
/// Represents rarely changing components of an in-flight async2 computation
[<NoEquality; NoComparison>]
[<AutoSerializable(false)>]
type Async2ActivationAux =
    {
...
/// Represents context for an in-flight async2 computation
[<NoEquality; NoComparison>]
[<AutoSerializable(false)>]
type Async2ActivationContents<'T> =
    {
        /// The success continuation
        cont: cont<'T>

        /// The rarely changing components
        aux: Async2ActivationAux
    }

/// A struct wrapper around Async2ActivationContents. Using a struct wrapper allows us to change representation of the
/// contents at a later point, e.g. to change the contents to a .NET Task or some other representation.
[<Struct; NoEquality; NoComparison>]
type Async2Activation<'T>(contents: Async2ActivationContents<'T>) =

    /// Produce a new execution context for a composite async2
    ...
    member _.WithContinuation cont =
        Async2Activation<'U> { cont = cont; aux = contents.aux }
    ...
    /// The extra information relevant to the execution of the async2
    member _.aux = contents.aux

    /// The success continuation relevant to the execution of the async2
    member _.cont = contents.cont

    /// The exception continuation relevant to the execution of the async2
    member _.econt = contents.aux.econt

    /// The cancellation continuation relevant to the execution of the async2
    member _.ccont = contents.aux.ccont

    /// The cancellation token relevant to the execution of the async2
    member _.token = contents.aux.token

    /// The trampoline holder being used to protect execution of the async2
    ...
    static member inline HijackCheckThenCall (ctxt: Async2Activation<'T>) cont arg =
    ...
    //
    // Note, this must make tailcalls, so may not be an instance member taking a byref argument.
    static member Success (ctxt: Async2Activation<'T>) result =
        if ctxt.IsCancellationRequested then
            ctxt.OnCancellation()
        else
            Async2Activation<'T>.HijackCheckThenCall ctxt ctxt.cont result

    // For backwards API Compat
    [<Obsolete("Call Success instead")>]
    member ctxt.OnSuccess(result: 'T) =
        Async2Activation<'T>.Success ctxt result

    /// Save the exception continuation during propagation of an exception, or prior to raising an exception
    member _.OnExceptionRaised() =
        contents.aux.trampolineHolder.OnExceptionRaised contents.aux.econt

    /// Make an initial async2 activation.
    static member Create cancellationToken trampolineHolder cont econt ccont : Async2Activation<'T> =
        Async2Activation
            {
                cont = cont
                aux =
    ...
    member ctxt.PostWithTrampoline (syncCtxt: SynchronizationContext) (f: unit -> Async2Return) =
...
[<NoEquality; NoComparison; CompiledName("FSharpAsync2`1")>]
type Async2<'T> =
    {
        Invoke: Async2Activation<'T> -> Async2Return
    }
...
type Async2Result<'T> =
    ...
    member res.Commit() =
        match res with
        | Async2Result.Ok res -> res
        | Async2Result.Error edi -> edi.ThrowAny()
        | Async2Result.Canceled exn -> raise exn
...
module Async2Primitives =
    ...
    let inline unfake (_: Async2Return) =
        ()

    /// The mutable global CancellationTokenSource, see Async2.DefaultCancellationToken
    let mutable defaultCancellationTokenSource = new CancellationTokenSource()

    /// Primitive to invoke an async2 computation.
    //
    // Note: direct calls to this function may end up in user assemblies via inlining
    [<DebuggerHidden>]
    let Invoke (computation: Async2<'T>) (ctxt: Async2Activation<_>) : Async2Return =
        Async2Activation<'T>.HijackCheckThenCall ctxt computation.Invoke ctxt
    ...
    [<DebuggerHidden>]
    let CallThenContinue userCode arg (ctxt: Async2Activation<_>) : Async2Return =
    ...
    [<DebuggerHidden>]
    let CallThenInvoke (ctxt: Async2Activation<_>) result1 part2 : Async2Return =
        let mutable result = Unchecked.defaultof<_>
        let mutable ok = false
    ...
    [<DebuggerHidden>]
    let CallThenInvokeNoHijackCheck (ctxt: Async2Activation<_>) result1 userCode =
    ...
    [<DebuggerHidden>]
    let CallFilterThenInvoke (ctxt: Async2Activation<'T>) filterFunction (edi: ExceptionDispatchInfo) : Async2Return =
        ...
        if ok then
            match resOpt with
            | None -> Async2Activation<'T>.HijackCheckThenCall ctxt ctxt.econt edi
    ...
    [<DebuggerHidden>]
    let Bind (ctxt: Async2Activation<'T>) (part1: Async2<'U>) (part2: 'U -> Async2<'T>) : Async2Return =
        if ctxt.IsCancellationRequested then
            ctxt.OnCancellation()
        else
    ...
    [<DebuggerHidden>]
    let TryFinally (ctxt: Async2Activation<'T>) (computation: Async2<'T>) finallyFunction =
    ...
    /// If it returns Some, invoke the resulting async2.
    ...
    [<DebuggerHidden>]
    let TryWith (ctxt: Async2Activation<'T>) (computation: Async2<'T>) catchFunction =
        if ctxt.IsCancellationRequested then
            ctxt.OnCancellation()
        else
            let ctxt =
    ...
    /// Make an async2 for an Async2Result
    //   - No cancellation check
    //   - No hijack check
    let CreateAsyncResultAsync res =
        MakeAsync(fun ctxt ->
            match res with
            | Async2Result.Ok r -> ctxt.cont r
            | Async2Result.Error edi -> ctxt.econt edi
            | Async2Result.Canceled cexn -> ctxt.ccont cexn)

    /// Generate async2 computation which calls its continuation with the given result
    ...
    let inline CreateReturnAsync res =
        ...
        MakeAsync(fun ctxt -> Async2Activation.Success ctxt res)
    ...
    /// Implements the sequencing construct of async2 computation expressions
    ...
    /// Create an async2 for a try/finally
    ///   - Cancellation check after 'entering' the try/finally and before running the body
    ...
    /// Create an async2 for a try/with filtering exceptions through a pattern match
    ...
    /// Create an async2 for a try/with filtering
    ///   - Cancellation check before entering the try (see TryWith)
    ///   - Cancellation check before entering the with (see TryWith)
    ...
    let CreateUsingAsync (resource: 'T :> IDisposable | null) (computation: 'T -> Async2<'a>) : Async2<'a> =
        let disposeFunction () =
    ...
    /// Implement the while loop construct of async2 computation expressions
    ...
    let CreateWhileAsync guardFunc computation =
        ...
        //
        // Note: There are allocations during loop set up, but no allocations during iterations of the loop
        // One allocation for While async2
        // One allocation for While async2 context function
    ...
    /// Implement the for loop construct of async2 computation expressions
    ...
    ///   - Do not apply 'GetEnumerator' with exception protection. However for an 'async2'
    ///     in an 'async2 { ... }' the exception protection will be provided by the enclosing
    ...
    let DelimitSyncContext (ctxt: Async2Activation<_>) =
    ...
    [<DebuggerHidden>]
    let StartWithContinuations cancellationToken (computation: Async2<'T>) cont econt ccont =
        let trampolineHolder = TrampolineHolder()

        trampolineHolder.ExecuteWithTrampoline(fun () ->
            let ctxt =
                Async2Activation.Create
                    cancellationToken
                    trampolineHolder
                    (cont >> fake)
                    (econt >> fake)
    ...
    [<Sealed>]
    [<AutoSerializable(false)>]
    type SuspendedAsync<'T>(ctxt: Async2Activation<'T>) =
    ...
    let QueueAsync cancellationToken cont econt ccont computation =
        ...
        trampolineHolder.QueueWorkItemWithTrampoline(fun () ->
            let ctxt =
                Async2Activation.Create cancellationToken trampolineHolder cont econt ccont
    ...
    let QueueAsyncAndWaitForResultSynchronously computation (token: CancellationToken) timeout =
        let token, innerCTS =
            // If timeout is provided, we govern the async2 by our own CTS, to cancel
            // when execution times out. Otherwise, the user-supplied token governs the async2.
            match timeout with
            | None -> token, None
            | Some _ ->
        ...
        use resultCell = new ResultCell<Async2Result<_>>()

        QueueAsync
            token
            (fun res -> resultCell.RegisterResult(Async2Result.Ok res, reuseThread = true))
            (fun edi -> resultCell.RegisterResult(Async2Result.Error edi, reuseThread = true))
            (fun exn -> resultCell.RegisterResult(Async2Result.Canceled exn, reuseThread = true))
            computation
        |> unfake
    ...
    [<DebuggerHidden>]
    let RunSynchronouslyBackgroundThreadPool (computation: Async2<'T>) cancellationToken timeout =
    ...
    [<DebuggerHidden>]
    let Start cancellationToken (computation: Async2<unit>) =
        QueueAsync
            cancellationToken
    ...
    [<DebuggerHidden>]
    let StartAsTask cancellationToken (computation: Async2<'T>) taskCreationOptions =
    ...
    // Used by Async2.Await path to elide egregious AggregateException wrapping
    [<DebuggerHidden>]
    ...
    [<DebuggerHidden>]
    let OnTaskCompleted unwrap (completedTask: Task<'T>) (ctxt: Async2Activation<'T>) =
    ...
    // the overall async2 (they may be governed by different cancellation tokens, or
    // the task may not have a cancellation token at all).
    [<DebuggerHidden>]
    let OnUnitTaskCompleted unwrap (completedTask: Task) (ctxt: Async2Activation<unit>) =
        assert completedTask.IsCompleted
    ...
    // execution of the async2 there.
    [<DebuggerHidden>]
    let AttachContinuationToTask unwrap (task: Task<'T>) (ctxt: Async2Activation<'T>) =
        task.ContinueWith(
    ...
    // execution of the async2 there.
    [<DebuggerHidden>]
    let AttachContinuationToUnitTask unwrap (task: Task) (ctxt: Async2Activation<unit>) =
        task.ContinueWith(
    ...
    /// Cleans up a Timer, helper for Async2.Sleep
    ...
    type Async2IAsyncResult<'T>(callback: AsyncCallback, state: objnull) =
        ...
        let result = new ResultCell<Async2Result<'T>>()

        member s.SetResult(v: Async2Result<'T>) =
        ...
        member s.GetResult() =
            ...
            | Some(Async2Result.Ok v) -> v
            | Some(Async2Result.Error edi) -> edi.ThrowAny()
            | Some(Async2Result.Canceled err) -> raise err
    ...
    module AsBeginEndHelpers =
        let beginAction (computation, callback, state) =
            let aiar = new Async2IAsyncResult<'T>(callback, state)

            let cont res =
                aiar.SetResult(Async2Result.Ok res)

            let econt edi =
                aiar.SetResult(Async2Result.Error edi)

            let ccont cexn =
                aiar.SetResult(Async2Result.Canceled cexn)
        ...
        let endAction<'T> (iar: IAsyncResult) =
            match iar with
            | :? Async2IAsyncResult<'T> as aiar ->
                if aiar.IsClosed then
                    raise (ObjectDisposedException("Async2Result"))
                else
        ...
        let cancelAction<'T> (iar: IAsyncResult) =
            match iar with
            | :? Async2IAsyncResult<'T> as aiar -> aiar.CancelAsync()
            | _ -> invalidArg "iar" (SR.GetString(SR.mismatchIARCancel))

open Async2Primitives
...
type Async2Builder() =
    ...
    member inline _.ReturnFrom(computation: Async2<_>) =
        computation
...
type Async2 =
    ...
    static member RunSynchronously(computation: Async2<'T>, ?timeout, ?cancellationToken: CancellationToken) =
    ...
    static member RunSynchronouslyImmediate(computation: Async2<'T>, ?cancellationToken: CancellationToken) =
    ...
    static member Parallel(computations: seq<Async2<'T>>) =
        Async2.Parallel(computations, ?maxDegreeOfParallelism = None)
    ...
    static member Sequential(computations: seq<Async2<'T>>) =
        Async2.Parallel(computations, maxDegreeOfParallelism = 1)
    ...
    static member StartWithContinuationsUsingDispatchInfo
        (computation: Async2<'T>, continuation, exceptionContinuation, cancellationContinuation, ?cancellationToken)
        : unit =
        let cancellationToken =
            defaultArg cancellationToken defaultCancellationTokenSource.Token
    ...
    static member StartWithContinuations
        (computation: Async2<'T>, continuation, exceptionContinuation, cancellationContinuation, ?cancellationToken)
        : unit =
        Async2.StartWithContinuationsUsingDispatchInfo(
            computation,
            continuation,
    ...
    static member AwaitIAsyncResult(iar: IAsyncResult, ?millisecondsTimeout) =
        async2 {
            if iar.CompletedSynchronously then
                ...
                return! Async2.AwaitWaitHandle(iar.AsyncWaitHandle, ?millisecondsTimeout = millisecondsTimeout)
        }

    /// Await and use the result of a result cell. The resulting async2 doesn't support cancellation
    /// or timeout directly, rather the underlying computation must fill the result if cancellation
    /// or timeout occurs.
    static member AwaitAndBindResult_NoDirectCancelOrTimeout(resultCell: ResultCell<Async2Result<'T>>) =
        async2 {
            let! result = resultCell.AwaitResult_NoDirectCancelOrTimeout
            return! CreateAsyncResultAsync result
        }

    /// Await the result of a result cell belonging to a child computation.  The resulting async2 supports timeout and if
    /// it happens the child computation will be cancelled.   The resulting async2 doesn't support cancellation
    ...
    static member AwaitAndBindChildResult
        (innerCTS: CancellationTokenSource, resultCell: ResultCell<Async2Result<'T>>, millisecondsTimeout)
        : Async2<'T> =
        match millisecondsTimeout with
        | None
        | Some -1 -> resultCell |> Async2.AwaitAndBindResult_NoDirectCancelOrTimeout
                ...
                try
                    if resultCell.ResultAvailable then
                        ...
                        if ok then
                            ...
                            let! _ = Async2.AwaitWaitHandle(resultCell.GetWaitHandle())
                            return raise (TimeoutException())
                finally
                    resultCell.Close()
            }
    ...
    static member FromBeginEnd(beginAction, endAction, ?cancelAction) : Async2<'T> =
        async2 {
            ...
            if iar.CompletedSynchronously then
                ...
                return! Async2.AwaitAndBindResult_NoDirectCancelOrTimeout resultCell
        }

    static member FromBeginEnd(arg, beginAction, endAction, ?cancelAction) : Async2<'T> =
        Async2.FromBeginEnd((fun (iar, state) -> beginAction (arg, iar, state)), endAction, ?cancelAction = cancelAction)
    ...
    static member AwaitEvent(event: IEvent<'Delegate, 'T>, ?cancelAction) : Async2<'T> =
        async2 {
            ...
            // Return the async2 computation that allows us to await the result
            // Note: ok to use "NoDirectCancel" here because cancellation has been registered above
            // Note: ok to use "NoDirectTimeout" here because no timeout parameter to this method
            return! Async2.AwaitAndBindResult_NoDirectCancelOrTimeout resultCell
        }
    ...
    static member StartChild(computation: Async2<'T>, ?millisecondsTimeout) =
        async2 {
            ...
            return Async2.AwaitAndBindChildResult(innerCTS, resultCell, millisecondsTimeout)
        }
    ...
    static member StartTaskImmediate(createTask: CancellationToken -> Task<'T>) : Async2<'T> =
        CreateBindAsync Async2.CancellationToken (createTask >> Async2.Await)
...
module Async2TaskLikeExtensions =
    ...
    type Async2 with
        ...
        static member inline Await< ^TaskLike, ^Awaiter, 'T
            ...
            (task: ^TaskLike)
            : Async2<'T> =
            Async2.FromContinuations(fun (cont, econt, _ccont) ->
...
module CommonExtensions =
    ...
    type System.IO.Stream with
        ...
        member stream.AsyncRead(buffer: byte array, ?offset, ?count) =
            ...
            Async2.FromBeginEnd(buffer, offset, count, stream.BeginRead, stream.EndRead)
        ...
        member stream.AsyncWrite(buffer: byte array, ?offset: int, ?count: int) =
            let offset = defaultArg offset 0
            let count = defaultArg count buffer.Length
            Async2.FromBeginEnd(buffer, offset, count, stream.BeginWrite, stream.EndWrite)
...
module WebExtensions =
        ...
        member req.AsyncGetResponse() : Async2<System.Net.WebResponse> =
            ...
            |> CreateTryWithFilterAsync(fun exn ->
                ...
                | :? System.Net.WebException as webExn when
                    ...
                    Some(CreateAsyncResultAsync(Async2Result.Canceled(OperationCanceledException webExn.Message)))
                | _ -> None)
    ...
    type System.Net.WebClient with
        ...
        member inline private this.Download(event: IEvent<'T, _>, handler: _ -> 'T, start, result) =
            let downloadAsync =
                Async2.FromContinuations(fun (cont, econt, ccont) ->
                    let userToken = obj ()
            ...
            async2 {
                use! _holder = Async2.OnCancel(fun _ -> this.CancelAsync())
        ...
        member this.AsyncDownloadData(address: Uri) : Async2<byte array> =
            this.Download(
...
module Async2 =
    ...
    [<CompiledName("Map")>]
    let inline map ([<InlineIfLambda>] mapping: 'T -> 'U) (computation: Async2<'T>) : Async2<'U> =
        async2.Bind(computation, mapping >> async2.Return)

    [<CompiledName("Bind")>]
    let inline bind ([<InlineIfLambda>] binder: 'T -> Async2<'U>) (computation: Async2<'T>) : Async2<'U> =
        async2.Bind(computation, binder)
    ...
    [<CompiledName("Empty")>]
    let empty: Async2<unit> = async2.Zero()

    [<CompiledName("SequentialDo")>]
    let sequentialDo (computations: seq<Async2<unit>>) : Async2<unit> =
        Async2.Sequential computations |> ignore<unit[]>

    [<CompiledName("ParallelLimit")>]
    let parallelLimit (maxDegreeOfParallelism: int) (computations: seq<Async2<'T>>) : Async2<'T[]> =
        Async2.Parallel(computations, maxDegreeOfParallelism = maxDegreeOfParallelism)

    [<CompiledName("ParallelDoLimit")>]
    let parallelDoLimit (maxDegreeOfParallelism: int) (computations: seq<Async2<unit>>) : Async2<unit> =
