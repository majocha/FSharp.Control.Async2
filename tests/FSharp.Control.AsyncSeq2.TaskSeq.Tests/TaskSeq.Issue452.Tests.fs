module AsyncSeq2.Tests.``Issue 452 -- external IAsyncEnumerable on custom scheduler``

// See https://github.com/fsprojects/FSharp.Control.AsyncSeq2/issues/452
//
// Report: wrapping combinators (AsyncSeq2.map, `asyncSeq2 { for .. in .. do yield .. }`) over an
...
// IAsyncEnumerable<'T> wrapped by AsyncSeq2.map and by `asyncSeq2 { for .. do yield .. }`, consumed
...
[<Fact>]
let ``AsyncSeq2.map over external IAsyncEnumerable on custom TaskScheduler does not duplicate last item`` () =
    let itemCount = 3

    let result =
        runOnCustomScheduler (fun () -> task {
            let upstream = ExternalEnumerable(itemCount) :> IAsyncEnumerable<int>
            let mapped = upstream |> AsyncSeq2.map (fun x -> x * 10)
            return! mapped |> ColdTask.toListAsync
        })
...
[<Fact>]
let ``asyncSeq2 { for .. in .. do yield } over external IAsyncEnumerable on custom TaskScheduler does not duplicate last item`` () =
    let itemCount = 3
    ...
    let result =
        runOnCustomScheduler (fun () -> task {
            ...
            let wrapped = asyncSeq2 {
                for x in upstream do
                    yield x * 10
            }

            return! wrapped |> ColdTask.toListAsync
        })
