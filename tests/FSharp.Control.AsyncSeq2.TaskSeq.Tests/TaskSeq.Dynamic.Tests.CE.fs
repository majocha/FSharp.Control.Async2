module AsyncSeq2.Tests.``taskSeqDynamic Computation Expression``
...
// taskSeqDynamic uses the same static path as asyncSeq2 when
...
[<Fact>]
let ``CE taskSeqDynamic yield from asyncSeq2`` () = task {
    let inner = asyncSeq2 {
        yield 1
        yield 2
    }
...
let ``CE taskSeqDynamic supports re-enumeration`` () = task {
    ...
    let! data1 = ts |> ColdTask.toListAsync
    let! data2 = ts |> ColdTask.toListAsync
