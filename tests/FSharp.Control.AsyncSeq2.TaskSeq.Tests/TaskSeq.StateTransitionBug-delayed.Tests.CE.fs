module AsyncSeq2.Tests.``Bug #42 -- asynchronous`` // see PR #42
...
let ``AsyncSeq2-toArray can be applied multiple times to the same sequence`` () =
    ...
    results2 |> should equal [| 1..7 |] // no mutable state in asyncSeq2, multi iter remains stable
    results3 |> should equal [| 1..7 |] // id
    results4 |> should equal [| 1..7 |] // id

// Note: this test used to hang (#42), please leave it in, no matter how silly it looks
[<Fact>]
let ``AsyncSeq2-toArray can be applied multiple times to the same sequence -- mutable state`` () =
    ...
    let (results1: _ list) = tq |> AsyncSeq2.toListSync
    let (results2: _ list) = tq |> AsyncSeq2.toListSync
    let (results3: _ list) = tq |> AsyncSeq2.toListSync
    let (results4: _ list) = tq |> AsyncSeq2.toListSync
