...
type ``Real world tests``(output: ITestOutputHelper) =
    [<Fact>]
    let ``Reading a 10MB buffered IAsync through AsyncSeq2.toArraySync non-async should succeed`` () = task {
        ...
        let results = reader |> AsyncSeq2.toArraySync
    ...
    let ``Reading a user-code IAsync multiple times with ColdTask.toArrayAsync should succeed`` () = task {
        use reader = AsyncBufferedReader(output, Array.init 2048 byte, 256)
        let expected = Array.init 256 byte |> Array.replicate 8
        // read four times
        let! results1 = reader |> ColdTask.toArrayAsync
        let! results2 = reader |> ColdTask.toArrayAsync
        let! results3 = reader |> ColdTask.toArrayAsync
        let! results4 = reader |> ColdTask.toArrayAsync
    ...
    let ``Reading a 10MB buffered IAsync stream from start to finish`` () = task {
        ...
        // reader |> AsyncSeq2.toArraySync |> should equal expected // VERY SLOW!!

        do! reader |> AsyncSeq2.iter (should equal expected) |> Async2.StartAsTask
        do! reader |> AsyncSeq2.iter ((=) expected >> (should be True)) |> Async2.StartAsTask
        let! len = reader |> AsyncSeq2.mapi (fun i _ -> i + 1) |> ColdTask.last
        len |> should equal 8
    ...
    //   at <StartupCode$FSharpy-AsyncSeq2-Test>.$AsyncSeq2.Realworld.clo@58-4.MoveNext() in D:\Projects\OpenSource\Abel\AsyncSeq2\src\FSharpy.AsyncSeq2.Test\AsyncSeq2.Realworld.fs:line 77
    ...
    let ``Reading a 1MB buffered IAsync stream from start to finish InvalidOperationException`` () = task {
        ...
        //do! ts |> AsyncSeq2.iter (should equal expected)
        do! ts |> AsyncSeq2.iter ((=) expected >> (should be True)) |> Async2.StartAsTask
        do! task { do count |> should equal 4096 }
    }
