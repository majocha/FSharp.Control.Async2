module AsyncSeq2.Tests.``asyncSeq2 Computation Expression``
...
[<Fact>]
let ``CE asyncSeq2 using yield! with null`` () = task {
    let ts = asyncSeq2 {
        yield! Gen.sideEffectTaskSeq 10
        yield! (null: AsyncSeq2<int>)
    }

    fun () -> AsyncSeq2.toListSync ts
    |> assertThrows typeof<NullReferenceException>
}

[<Fact>]
let ``CE asyncSeq2 with several yield!`` () = task {
    ...
    let! data = tskSeq |> ColdTask.toListAsync
...
[<Fact>]
let ``CE asyncSeq2 with nested yield!`` () = task {
    let control = seq {
        yield! [ 1..10 ]
    ...
    let tskSeq = asyncSeq2 {
        ...
        for _ in 0..9 do
            ...
            for _ in 0..2 do
                yield! asyncSeq2 { yield 42 }

                for i in 100..102 do
                    yield! asyncSeq2 { yield! asyncSeq2 { yield i } }
    }
...
[<Fact>]
let ``CE asyncSeq2 with nested deeply yield! perf test 8521 nested tasks`` () = task {
    let expected = seq {
        yield! [ 1..10 ]
...
[<Fact>]
let ``CE asyncSeq2 with mixing yield! and yield`` () = task {
    let tskSeq = asyncSeq2 {
        yield! Gen.sideEffectTaskSeq 10
        yield 42
        yield! Gen.sideEffectTaskSeq 5
...
[<Fact>]
let ``CE asyncSeq2: 1000 TaskDelay-delayed tasks using yield!`` () = task {
    ...
    let tskSeq = asyncSeq2 { yield! Gen.sideEffectTaskSeqMicro 50L<µs> 1000L<µs> 1000 }
    let! data = tskSeq |> ColdTask.toListAsync
    data |> should equal [ 1..1000 ]
}

[<Fact>]
let ``CE asyncSeq2: 1000 sync-running tasks using yield!`` () = task {
    // Smoke performance test
    // Runs in a few 10's of ms, because of absence of Task.Delay
    // should generally be about as fast as `task`, see below
    let tskSeq = asyncSeq2 { yield! Gen.sideEffectTaskSeq_Sequential 1000 }
    let! data = tskSeq |> ColdTask.toListAsync
    data |> should equal [ 1..1000 ]
}

[<Fact>]
let ``CE asyncSeq2: 5000 sync-running tasks using yield!`` () = task {
    // Smoke performance test
    // Compare with task-ce test below. Uses a no-delay hot-started sequence of tasks.
    let tskSeq = asyncSeq2 { yield! Gen.sideEffectTaskSeq_Sequential 5000 }
    let! data = tskSeq |> ColdTask.toListAsync
...
let ``CE task: 1000 TaskDelay-delayed tasks using for-loop`` () = task {
    ...
    // for smoke-test comparison with asyncSeq2
